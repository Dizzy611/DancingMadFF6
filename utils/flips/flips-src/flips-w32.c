...
struct mem ReadWholeFile(LPCWSTR filename)
{
	...
	if (file==INVALID_HANDLE_VALUE) return (struct mem){NULL, 0};
	DWORD len=GetFileSize(file, NULL);
	unsigned char * data=(unsigned char*)malloc(len);
	ReadFile(file, data, len, &len/*looks like this optional parameter isn't optional after all. fun*/, NULL);
	CloseHandle(file);
...
bool WriteWholeFile(LPCWSTR filename, struct mem data)
{
	...
	DWORD truelen;
	WriteFile(file, data.ptr, data.len, &truelen, NULL);
	CloseHandle(file);
...
bool WriteWholeFileWithHeader(LPCWSTR filename, struct mem header, struct mem data)
{
	...
	if (file==INVALID_HANDLE_VALUE) return false;
	DWORD truelen;
	WriteFile(file, header.ptr, 512, &truelen, NULL);
	WriteFile(file, data.ptr, data.len, &truelen, NULL);
	CloseHandle(file);
	return (truelen==data.len);
}
...
bool SelectRom(LPWSTR filename, LPCWSTR title, bool output)
{
	...
	ofn.lpstrFilter=TEXT("Most Common ROM Files\0*.smc;*.sfc;*.nes;*.gb;*.gbc;*.gba;*.vb;*.sms;*.smd;*.ngp\0All Files (*.*)\0*.*\0");
	ofn.lpstrFile=filename;
	ofn.nMaxFile=MAX_PATH;
	ofn.nFilterIndex=state.lastRomType;
	ofn.lpstrTitle=title;
	ofn.Flags=OFN_HIDEREADONLY|OFN_FILEMUSTEXIST|OFN_PATHMUSTEXIST|(output?OFN_OVERWRITEPROMPT:0);
...
UINT mboxtype[]={ MB_OK, MB_OK|MB_ICONWARNING, MB_OK|MB_ICONWARNING, MB_OK|MB_ICONERROR, MB_OK|MB_ICONERROR };
...
int a_ApplyPatch(LPCWSTR clipatchname)
{
	...
	if (!multiplePatches)
	{
		...
		inromname[0]='\0';
		if (!SelectRom(inromname, TEXT("Select File to Patch"), false)) return 0;
		WCHAR outromname[MAX_PATH];
		wcscpy(outromname, inromname);
		PWCHAR outrompath=GetBaseName(outromname);
		PWCHAR patchpath=GetBaseName(patchnames);
		if (outrompath && patchpath)
		{
			wcscpy(outrompath, patchpath);
			PWCHAR outromext=GetExtension(outrompath);
			PWCHAR inromext=GetExtension(inromname);
			if (*inromext && *outromext) wcscpy(outromext, inromext);
		}
		if (!SelectRom(outromname, TEXT("Select Output File"), true)) return 0;
		struct errorinfo errinf=ApplyPatch(patchnames, inromname, outromname, NULL);
	...
	else
	{
		...
		PWCHAR thisFileName=wcschr(thisFileNameWithPath, '\0');
		*thisFileName='\\';
		thisFileName++;
		PWCHAR thisPatchName=wcschr(patchnames, '\0')+1;
		LPCWSTR romExtension=GetExtension(inromname);
		struct mem inrom=ReadWholeFile(inromname);
		bool anySuccess=false;
		enum { e_none, e_warning, e_invalid_this, e_invalid, e_io_write, e_io_read, e_io_read_rom } worsterror=e_none;
		enum errorlevel severity[2][8]={
				 { el_ok,  el_warning,el_warning,el_broken,      el_broken, el_broken,  el_broken, el_broken },
				 { el_ok,  el_warning,el_warning,el_warning,     el_warning,el_warning, el_warning,el_broken },
			};
		LPCSTR messages[2][8]={
				{
					//no error-free
					NULL,//e_none
					NULL,//e_warning_notthis
					NULL,//e_warning
					...
					"The patches were applied, but one or more is unlikely to be intended for this ROM...",//e_warning_notthis
		...
		if (inrom.ptr)
		{
			bool removeheaders=shouldRemoveHeader(inromname, inrom);
			while (*thisPatchName)
			{
				wcscpy(thisFileName, thisPatchName);
				struct mem patch=ReadWholeFile(thisFileNameWithPath);
				if (patch.ptr)
				{
					PWCHAR patchExtension=GetExtension(thisFileName);
					wcscpy(patchExtension, romExtension);
					struct errorinfo errinf=ApplyPatchMem(patch, inrom, removeheaders, thisFileNameWithPath, NULL);
...
void a_CreatePatch()
{
	...
	ofn.lpstrFilter=TEXT(
											 "IPS Patch File (*.ips)\0*.ips\0"
											 "BPS Patch File (Favor Creation Speed) (*.bps)\0*.bps\0"
											 "BPS Patch File (Favor Small Size) (*.bps)\0*.bps\0"
											 );
	...
	bpsdCancel=false;
	struct errorinfo errinf=CreatePatch(romnames[0], romnames[1], (enum patchtype)ofn.nFilterIndex, patchname, NULL);
...
bool a_SetEmulator()
{
	...
	*newemupath='\0';
	...
	wcscpy(state.emulator, newemupath);
	return true;
}

int a_ApplyRun(LPCWSTR clipatchname)
{
	struct mem patchmem={NULL,0};
	...
	*patchpath='\0';
	...
	else
	{
		...
		ofn.lStructSize=sizeof(ofn);
		ofn.hwndOwner=hwndMain;
		ofn.lpstrFilter=TEXT("All supported patches (*.ips, *.bps)\0*.ips;*.bps\0All files (*.*)\0*.*\0");
	...
	*romname='\0';
	if (!SelectRom(romname, TEXT("Select Base File"), false)) return 0;
	
	if (!*state.emulator && !a_SetEmulator()) return 0;
	...
	struct errorinfo errinf=ApplyPatch(patchpath, romname, tempfilename, NULL);
	...
	if (patchmem.ptr) FreeFileMemory(patchmem);
	if (rommem.ptr) FreeFileMemory(rommem);
	if (patchedmem.ptr) free(patchedmem.ptr);
	
	WCHAR cmdline[1+MAX_PATH+3+MAX_PATH+1+1];
	swprintf(cmdline, TEXT("\"%s\" \"%s\""), state.emulator, tempfilename);
	WCHAR * dirend=GetBaseName(patchpath);
	if (dirend) *dirend='\0';
	STARTUPINFO startupinfo;
	ZeroMemory(&startupinfo, sizeof(STARTUPINFO));
	PROCESS_INFORMATION processinformation;
	if (!CreateProcess(state.emulator, cmdline, NULL, NULL, FALSE, 0, NULL, patchpath, &startupinfo, &processinformation))
	{
		MessageBoxA(hwndMain, "Couldn't open emulator.", flipsversion, mboxtype[el_broken]);
		DeleteFile(tempfilename);
		return el_broken;
	}
	CloseHandle(processinformation.hProcess);//I don't clean up the temp file when the emulator is done. It'd just force me to keep track of which files
	CloseHandle(processinformation.hThread);//are created, as well as not exiting when the window is closed, so I don't care. Few other apps clean up anyways.
	return errinf.level;
}
...
void a_ShowSettings()
{
	...
	hwndSettings=CreateWindowA(
				...
				CW_USEDEFAULT, CW_USEDEFAULT, 3+6+202+6+3, 21+96+3, NULL, NULL, GetModuleHandle(NULL), NULL);
...
#define firstbutton(text, w, h, action) \
			widget("BUTTON", WS_GROUP|BS_DEFPUSHBUTTON, text, w, h, action)
	
#define button(text, w, h, action) \
			widget("BUTTON", BS_PUSHBUTTON, text, w, h, action)
	
#define labelL(text, w, h, action) \
			widget("STATIC", SS_LEFT, text, w, h, action)
#define labelC(text, w, h, action) \
			widget("STATIC", SS_CENTER, text, w, h, action)
	
#define radio(text, w, h, action) \
			widget("BUTTON", BS_AUTORADIOBUTTON, text, w, h, action)
...
void a_AssignFileTypes(bool checkonly)
{
	...
	PWCHAR outstringend=wcschr(outstring, '\0');
	*outstringend='"';
	outstringend++;
...
#define button(x,y,w,h, text) \
				do { \
					lastbutton=CreateWindowA("BUTTON", text, WS_CHILD|WS_TABSTOP|WS_VISIBLE|(buttonid==0?(BS_DEFPUSHBUTTON|WS_GROUP):(BS_PUSHBUTTON)), \
													x, y, w, h, hwndMain, (HMENU)(buttonid+1), GetModuleHandle(NULL), NULL); \
...
int ShowGUI(LPCWSTR filename)
{
	...
	HANDLE file=CreateFile(cfgfname, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL);
	if (file!=INVALID_HANDLE_VALUE)
	{
		DWORD whocares;
		memset(&state, 0, sizeof(state));
		ReadFile(file, &state, sizeof(state), &whocares, NULL);
		CloseHandle(file);
	}
	if (strncmp(state.signature, "Flipscfg", sizeof(state.signature)) || state.cfgversion!=2)
	{
		strncpy(state.signature, "Flipscfg", sizeof(state.signature));
		state.cfgversion=2;
		state.lastRomType=0;
		state.lastPatchType=ty_null;
		state.emulator[0]='\0';
	...
	file=CreateFile(cfgfname, GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_HIDDEN, NULL);
	if (file!=INVALID_HANDLE_VALUE)
	{
		DWORD whocares;
		WriteFile(file, &state, sizeof(state)-sizeof(state.emulator)+(wcslen(state.emulator)+1)*2, &whocares, NULL);
		CloseHandle(file);
	}
